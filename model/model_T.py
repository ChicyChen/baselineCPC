...
# baseline 2D
class CPC_1layer_2d_static_T1(nn.Module):
    def __init__(self, code_size=512, top_size=512, pred_step=3, nsub=3, useout=False, seeall=False, window=1):
        super().__init__()
    ...
    def forward(self, block):
        ...
        if self.seeall:
            output, hidden = self.auto_agressive(feature)
            output = output[-1] # B, N, self.code_size[0], 7, 7
            pred = self.latent_pred(output.view(B*N, self.top_size, 7, 7)).view(B, N, self.code_size, 7, 7)
        ...
        # add a temporal window is like do convolution of similarity with a special filter
        # either [1/w, 1/w, ...], or in the shape of a gaussian
